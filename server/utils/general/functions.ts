...
const useGetVapidDetails = async () => {
	...
	webpush.setVapidDetails("mailto:example@yourdomain.org", vapidPublicKey, vapidPrivateKey);
...
export const useSendServiceWorkerPushEvent = async (payload: any) => {
	...
	if (!error)
		for (const data of subscriptions) {
			...
			webpush.sendNotification({ endpoint: endpoint, keys: keys }, JSON.stringify(payload)).catch(async () => {
