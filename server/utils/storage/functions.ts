...
const STORAGE_CACHE_STORAGE_KEYS = [
    `/cache:nitro/functions:${STORAGE_CACHE_NAME}:${STORAGE_CACHE_KEY}.json`,
    `nitro:functions:${STORAGE_CACHE_NAME}:${STORAGE_CACHE_KEY}.json`
];

const fetchAllStorageFiles = async (_key: string, server: SupabaseClient, _revalidate = false) => {
    const { data: attachments, error } = await server
        .from('attachments')
        .select('id, name, published, label, article_id, updated_at')
...
const filterStorageFiles = (files: any[], search: string, filter: string) => {
    ...
    return files.filter((file: any) => {
        const labelMatch = !filter || filter === 'alles' ? true
            : String(file.metadata?.label || '').toLowerCase().includes(filter.toLowerCase());
